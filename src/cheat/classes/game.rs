...
use crate::utils::cheat::process::{get_process_module_handle, rpm_auto};
...
pub fn update_entity_list_entry() -> bool {
    ...
    // Read entity_list value once (matching cs2_cheat approach)
    // cs2_cheat: entity_list = Read(modBase + dwEntityList)
    // Then in loop uses: entity_list + 0x10 + 0x8 * (i >> 9)
