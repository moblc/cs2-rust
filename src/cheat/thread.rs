    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            for i in 1 .. 64 {
                ...
                // Step 1: Resolve controller address using cs2_cheat method
                // listEntry = Read(entity_list + 0x10 + 0x8 * (i >> 9))
                // controller = Read(listEntry + 0x70 * (i & 0x1FF))
                let mut list_entry: u64 = 0;
                let mut entity_address: u64 = 0;
                
                // Read first level of entity list
                if !rpm_offset(game.address.entity_list_entry, 0x10 + 0x8 * ((i as u64) >> 9), &mut list_entry) {
                    remove_esp(i);
                    continue;
                }
                ...
                // Calculate controller address (using 0x70 not 0x78!)
                if !rpm_offset(list_entry, 0x70 * ((i as u64) & 0x1FF), &mut entity_address) {
                    remove_esp(i);
                    continue;
                }
